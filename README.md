...
4.1. **(Opcional/legado) Crear el usuario "revendedor" compartido** — con el sistema actual no
   hace falta: los revendedores se crean desde el panel del sitio (ver sección 2, más abajo),
   cada uno con su propio usuario y contraseña. Este paso solo tiene sentido si preferís seguir
   usando una única clave compartida entre todos, a la vieja usanza: repetí el paso 4 con Email
   `revendedor@ps2gamepass.local` (igual al `REVENDEDOR_EMAIL` de `supabase-config.js`) y la
   contraseña que quieras compartir, marcando "Auto Confirm User" si aparece la opción.
4.2. **Desactivar la confirmación de email (necesario para crear revendedores desde la
   página)** — Menú lateral → **Authentication → Providers → Email** (en proyectos nuevos puede
   aparecer como **Authentication → Sign In / Providers → Email**) → desmarcá **"Confirm email"**
   (a veces aparece como "Enable email confirmations") → Guardar. Como los revendedores se crean
   con un email inventado (no uno real), si esto queda activado no van a poder iniciar sesión
   nunca, porque el mail de confirmación no le llega a nadie.
...
### Agregar revendedores — se hace desde la propia página, con la clave general

No hay que crear nada a mano en Supabase para cada revendedor. Con la clave de editor
principal puesta:

1. Abrí el botón **"📋 Revendedores"** en la barra de herramientas.
2. Al final del panel, completá **Nombre** (para identificarlo, ej: "Juan - Zona Norte"),
   **Usuario** (ej: `juan`, sin espacios) y **Contraseña**, y tocá **"➕ Crear cuenta de
   revendedor"**.
3. Pasale a esa persona el usuario y la contraseña que elegiste. Va a entrar con el botón
   **"🏪 Modo revendedor"**, poniendo esos dos datos (no el "editor@..." ni la clave general).

Cada revendedor tiene su **propio usuario y contraseña** — no comparten uno solo. Podés crear
tantos como quieras, y en cualquier momento **activar o desactivar** el acceso de cualquiera
desde el mismo panel (botón "⛔ Desactivar" / "✅ Activar"), sin borrar nada ni tener que entrar
a Supabase. Una cuenta desactivada no puede escribir nada, aunque la persona siga iniciando
sesión con su usuario y contraseña — la base de datos lo rechaza del lado del servidor.

### Modo revendedor — acceso restringido

- Botón **"🏪 Modo revendedor"** → pide usuario y contraseña (los que le creaste en el panel de
  arriba) → si son correctos, esa persona puede:
  - Cambiar el **nombre de la tienda** de cualquier disco (tocando el lápiz ✏️ junto al título
    del disco que está viendo). Los discos son compartidos: no están "asignados" a un
    revendedor en particular, cualquier revendedor activo puede tocar cualquier disco.
...
- **No puede** cambiar nombres de juegos, imágenes, capturas, el WhatsApp, crear/borrar discos
  ...
  del lado del servidor (ver `migracion_revendedores_multiples.sql` / `schema.sql`).
...
herramientas: arriba, una lista con el nombre de tienda y el WhatsApp de cada disco, para
revisarlos o corregirlos todos juntos sin tener que entrar disco por disco; abajo, la lista de
**cuentas de revendedor** con el botón para crear una nueva y para activar/desactivar las que
ya existen (ver dos secciones más arriba). Ese botón no aparece nunca en modo revendedor ni
para un visitante.

### Cómo funciona técnicamente

La clave de editor inicia sesión contra el usuario fijo `editor@ps2gamepass.local`. Cada
revendedor tiene su propio usuario de Supabase Authentication, creado al vuelo desde el panel
(sin necesidad de tocar Supabase a mano); una tabla `revendedores` guarda su nombre y si está
activo. Nadie puede escribir sin ser el editor o un revendedor activo, aunque mire el código
del sitio o llame a la API a mano: la base de datos lo rechaza del lado del servidor (reglas
RLS + un trigger en `schema.sql`/`migracion_revendedores_multiples.sql`), no es una restricción
que se pueda saltear editando el HTML.

**Si tu proyecto de Supabase ya existía antes de este cambio:** corré una vez el contenido de
`migracion_revendedores_multiples.sql` en el SQL Editor, y no te olvides del paso 4.2 (desactivar
"Confirm email"). Esa misma migración de paso también agrega la columna `whatsapp` a `discos`
por si tu proyecto no la tenía (un descuido de una versión anterior). El viejo usuario compartido
`revendedor@ps2gamepass.local`, si ya lo tenías creado, sigue funcionando igual que antes, por
compatibilidad — pero para revendedores nuevos conviene usar el panel.

Si en algún momento querés cambiar la clave del editor principal: Authentication → Users →
click en el usuario `editor@ps2gamepass.local` → "Reset password". Para un revendedor, es más
simple desactivarlo desde el panel y crearle una cuenta nueva con otra contraseña.
...
migracion_revendedor.sql → Migración vieja: activa el modo revendedor con clave compartida (solo si tu proyecto es de antes)
migracion_precio_base.sql → Migración: agrega el piso de precio "página principal" (solo si tu proyecto es de antes)
migracion_revendedores_multiples.sql → Migración: cuentas de revendedor propias, creadas desde la página (solo si tu proyecto es de antes)

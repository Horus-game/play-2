...
function renderBanner() {
  ...
  $("#discBanner").innerHTML = `
    <h1>${d.nombre} · ${d.subtitulo || ""} <button class="edit-disc-btn admin-only" id="editDiscBtn" title="Editar nombre del disco">✏️</button></h1>
    <p>Insertá este disco en la PS2 y navegá su catálogo sin adivinar qué contiene.</p>
    <div class="count">${count}</div>
  `;
  const editBtn = $("#editDiscBtn");
  if (editBtn) editBtn.onclick = () => editDisco(d);
}
...
async function editDisco(d) {
  if (!isEditor) return;
  const nombre = window.prompt("Nombre del disco:", d.nombre);
  if (nombre === null) return;
  const subtitulo = window.prompt("Subtítulo:", d.subtitulo || "");
  if (subtitulo === null) return;
  const color = window.prompt("Color (código hex):", d.color || "#7c5cff");
  if (color === null) return;
  const { error } = await sb.from("discos").update({ nombre: nombre.trim() || d.nombre, subtitulo: subtitulo.trim(), color: color.trim() || d.color }).eq("id", d.id);
  if (error) { alert("No se pudo guardar. ¿Iniciaste sesión con la clave?"); console.error(error); return; }
  await reload();
}
  if (!isEditor) return;

...
let isEditor = false;
let isRevendedor = false; // sesión restringida: solo nombre de tienda + precio
let currentPriceMin = null; // filtro de precio: mínimo (null = sin límite)
let currentPriceMax = null; // filtro de precio: máximo (null = sin límite)
let selectionMode = false;  // modo "seleccionar varios juegos" (solo editor) para poner precio en lote
let SELECTED_IDS = new Set();

// Icono de WhatsApp en SVG (path oficial de Font Awesome, licencia CC BY 4.0),
// para no depender de una fuente de íconos externa que podría no cargar.
const WHATSAPP_ICON_SVG = `<svg class="wa-icon" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zM223.9 431.7h-.1c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 351.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.7z"/></svg>`;
...
async function reload() {
  await loadData();
  if (selectionMode) {
    const existingIds = new Set(JUEGOS.map(g => g.id));
    SELECTED_IDS.forEach(id => { if (!existingIds.has(id)) SELECTED_IDS.delete(id); });
    renderBulkBar();
  }
  renderConsolaSelect();
  renderDiscTabs();
...
// ---------- Auth / modo edición ----------
let revendedorNombre = null; // nombre de la cuenta de revendedor conectada (si aplica)

async function applySession(session) {
  const email = session && session.user && session.user.email;
  isEditor = !!session && email === window.EDITOR_EMAIL;
  isRevendedor = false;
  revendedorNombre = null;
  if (!isEditor && session && session.user) {
    if (email === window.REVENDEDOR_EMAIL) {
      // Cuenta compartida de siempre (por compatibilidad con proyectos previos).
      isRevendedor = true;
      revendedorNombre = "Revendedor";
    } else {
      const { data } = await sb.from("revendedores").select("nombre,activo").eq("user_id", session.user.id).maybeSingle();
      if (data && data.activo) { isRevendedor = true; revendedorNombre = data.nombre; }
    }
  }
}

// ---------- Panel oculto de revendedores (solo editor principal) ----------
// Muestra, en un solo lugar, el nombre de tienda y el WhatsApp de cada disco
// (= cada revendedor), para que el editor principal los revise o cambie
// cuando haga falta sin tener que ir disco por disco.
function openRevendedoresPanel() {
  if (!isEditor) return;
  renderRevendedoresPanel();
  loadRevendedoresAccounts();
  $("#revendedoresModalOverlay").classList.add("open");
  pushModalHistoryState();
}
function closeRevendedoresPanelUI() { $("#revendedoresModalOverlay").classList.remove("open"); }
function closeRevendedoresPanel() { requestCloseViaBack(closeRevendedoresPanelUI); }

function renderRevendedoresPanel() {
  const list = $("#revendedoresList");
  if (!DISCOS.length) { list.innerHTML = `<p class="dup-summary">Todavía no hay discos/revendedores creados.</p>`; return; }
  list.innerHTML = DISCOS.map(d => `
    <div class="revendedores-panel-row" data-disco-id="${d.id}">
      <label>Nombre de la tienda
        <input type="text" class="rev-input-nombre" value="${(d.nombre || "").replace(/"/g, "&quot;")}">
      </label>
      <label>WhatsApp (solo números)
        <input type="text" class="rev-input-whatsapp" value="${(d.whatsapp || "").replace(/"/g, "&quot;")}" placeholder="Ej: 5491122334455">
      </label>
      <button class="btn-save rev-save-btn" data-save-disco="${d.id}">💾 Guardar</button>
    </div>
  `).join("");
}

// ---------- Cuentas de revendedor (usuario + contraseña propios) ----------
async function loadRevendedoresAccounts() {
  const box = $("#revendedoresAccountsList");
  if (!box) return;
  box.innerHTML = `<p class="dup-summary">Cargando...</p>`;
  const { data, error } = await sb.from("revendedores").select("*").order("creado_en", { ascending: false });
  if (error) { box.innerHTML = `<p class="dup-summary">No se pudo cargar la lista: ${error.message || error.code}</p>`; return; }
  if (!data || !data.length) { box.innerHTML = `<p class="dup-summary">Todavía no creaste ninguna cuenta de revendedor.</p>`; return; }
  box.innerHTML = data.map(r => `
    <div class="revendedores-panel-row" data-revendedor-id="${r.id}">
      <div>
        <strong>${(r.nombre || "").replace(/</g, "&lt;")}</strong><br>
        <span class="dup-summary" style="margin:0;">${r.email}${r.activo ? "" : " · desactivada"}</span>
      </div>
      <button class="btn-save" data-toggle-revendedor="${r.id}" data-activo="${r.activo}">
...
async function submitNuevoRevendedor() {
  const nombre = $("#nuevoRevendedorNombre").value.trim();
  const usuario = $("#nuevoRevendedorUsuario").value.trim();
  const pass = $("#nuevoRevendedorPass").value;
  const btn = $("#crearRevendedorBtn");
  btn.disabled = true;
  const { ok, error, email, necesitaConfirmar } = await crearRevendedor(nombre, usuario, pass);
  btn.disabled = false;
  if (error) { alert(error); return; }
  $("#nuevoRevendedorNombre").value = "";
  $("#nuevoRevendedorUsuario").value = "";
  $("#nuevoRevendedorPass").value = "";
  if (necesitaConfirmar) {
    alert(`Cuenta creada (${email}), pero tu proyecto de Supabase pide confirmar el email antes de poder usarla. Andá a Authentication → Providers → Email y desactivá "Confirm email" (ver README).`);
  } else {
    alert(`Cuenta creada: ${email}. Pasale ese usuario ("${usuario}") y la contraseña al revendedor.`);
  }
  loadRevendedoresAccounts();
}

async function saveRevendedorRow(discoId) {
  const row = document.querySelector(`.revendedores-panel-row[data-disco-id="${CSS.escape(String(discoId))}"]`);
  if (!row) return;
  const nombre = row.querySelector(".rev-input-nombre").value.trim();
  const whatsapp = row.querySelector(".rev-input-whatsapp").value.trim().replace(/[^0-9]/g, "") || null;
  if (!nombre) { alert("El nombre no puede quedar vacío."); return; }
  const { error } = await sb.from("discos").update({ nombre, whatsapp }).eq("id", discoId);
...
async function unlockEditor() {
  ...
  const pass = window.prompt("Clave de edición (editor principal):");
  if (!pass) return;
...
// Login restringido: solo puede cambiar el nombre de la tienda (disco) y el
// precio de los juegos — todo lo demás lo bloquea la base de datos aunque se
// intente saltear la web (ver migracion_revendedor.sql / es_revendedor_activo()).
async function unlockRevendedor() {
  if (isRevendedor) {
    await sb.auth.signOut();
    return;
  }
  const usuario = window.prompt("Usuario de revendedor:");
  if (!usuario) return;
  const pass = window.prompt("Contraseña:");
  if (!pass) return;
  const email = revendedorEmailFromUsuario(usuario);
  const { error } = await sb.auth.signInWithPassword({ email, password: pass });
  if (error) {
    alert("Usuario o clave incorrectos, o la cuenta todavía no está creada (pedísela al editor principal).");
    console.error(error);
  }
}

// A partir de un "usuario" simple (ej: "juan") arma el email interno que usa
// Supabase Auth (ej: "juan@ps2gamepass.local"). Si ya escriben un email
// completo (tiene "@"), se respeta tal cual.
function revendedorEmailFromUsuario(usuario) {
  const limpio = usuario.trim().toLowerCase();
  if (limpio.includes("@")) return limpio;
  const slug = limpio.replace(/[^a-z0-9._-]/g, "");
  return `${slug}@ps2gamepass.local`;
}

// ---------- Crear y administrar cuentas de revendedor (solo editor principal) ----------
// Se crean sin salir del sitio: el editor tiene que estar logueado con la
// clave general para ver el panel y este formulario. Usamos un cliente de
// Supabase aparte (sin persistir sesión) para el alta, así el signUp no pisa
// la sesión de editor que ya está activa en este navegador.
async function crearRevendedor(nombre, usuario, password) {
  if (!isEditor) return { error: "No autorizado." };
  if (!nombre || !usuario || !password) return { error: "Completá nombre, usuario y contraseña." };
  if (password.length < 6) return { error: "La contraseña tiene que tener al menos 6 caracteres." };
  const email = revendedorEmailFromUsuario(usuario);
  const tempClient = createClient(window.supabaseConfig.url, window.supabaseConfig.anonKey, {
    auth: { persistSession: false, autoRefreshToken: false },
  });
  const { data, error } = await tempClient.auth.signUp({ email, password });
  if (error) return { error: error.message || "No se pudo crear la cuenta." };
  if (!data.user) return { error: "No se pudo crear la cuenta (respuesta inesperada de Supabase)." };
  const { error: insertError } = await sb.from("revendedores").insert({
    user_id: data.user.id, email, nombre: nombre.trim(), activo: true,
  });
  if (insertError) return { error: `La cuenta se creó pero no se pudo registrar: ${insertError.message || insertError.code}` };
  const necesitaConfirmar = !data.user.confirmed_at && !data.session;
  return { ok: true, email, necesitaConfirmar };
}

async function toggleRevendedorActivo(id, activo) {
  if (!isEditor) return;
  const { error } = await sb.from("revendedores").update({ activo: !activo }).eq("id", id);
  if (error) { alert(`No se pudo actualizar: ${error.message || error.code || "error desconocido"}`); console.error(error); return; }
  await renderRevendedoresPanel();
}

function applyEditorUI() {
  document.body.classList.toggle("is-editor", isEditor);
  document.body.classList.toggle("is-revendedor", isRevendedor);
  const editBtn = $("#unlockBtn");
  editBtn.title = isEditor ? "Editor principal (tocá para salir)" : "Editor principal";
  editBtn.classList.toggle("editor-active", isEditor);
  const revBtn = $("#unlockRevendedorBtn");
  if (revBtn) {
    revBtn.title = isRevendedor ? "Modo revendedor (tocá para salir)" : "Modo revendedor";
    revBtn.classList.toggle("editor-active", isRevendedor);
    revBtn.style.display = isEditor ? "none" : "";
  }
  updateDeletePlatformBtn();
  // Al salir de cualquier modo de edición: apagar "seleccionar juegos" si
  // estaba activo, y limpiar filtro/orden por precio (los precios quedan
  // ocultos para quien no está editando ni es revendedor).
  if (!isEditor && !isRevendedor) {
    if (selectionMode) { selectionMode = false; SELECTED_IDS.clear(); renderBulkBar(); }
    if (currentPriceMin != null || currentPriceMax != null) {
      currentPriceMin = null; currentPriceMax = null; syncPriceInputsFromState(); renderPricePresets();
    }
    if (currentSort === "precio-asc" || currentSort === "precio-desc") {
      currentSort = "alpha"; $("#sortSelect").value = "alpha";
    }
  }
  $("#sortPrecioAsc").style.display = (isEditor || isRevendedor) ? "" : "none";
  $("#sortPrecioDesc").style.display = (isEditor || isRevendedor) ? "" : "none";
  renderActiveFilters();
  renderBanner();
  renderGrid();
}
...
function updateDeletePlatformBtn() {
  ...
  if (!btn) return;
  btn.style.display = (isEditor && currentConsola !== ALL_ID) ? "inline-flex" : "none";
}
...
function renderBanner() {
  ...
  $("#discBanner").innerHTML = `
    <h1>${d.nombre} · ${d.subtitulo || ""} <button class="edit-disc-btn admin-only" id="editDiscBtn" title="Editar disco">✏️</button><button class="edit-disc-btn revendedor-only" id="renameStoreBtn" title="Cambiar el nombre de tu tienda">✏️</button><button class="edit-disc-btn admin-only" id="importGamesOpenBtn" title="Importar juegos con IA">📥</button></h1>
  ...
  const renameBtn = $("#renameStoreBtn");
  if (renameBtn) renameBtn.onclick = () => renameStore(d);
  const importBtn = $("#importGamesOpenBtn");
  if (importBtn) importBtn.onclick = () => openImportModal(d.id);
}

// El revendedor solo puede cambiar el nombre de la tienda (nombre del disco).
// La base de datos bloquea, del lado del servidor, cualquier otro cambio.
async function renameStore(d) {
  if (!isRevendedor) return;
  const nuevo = window.prompt("Nombre de tu tienda:", d.nombre);
  if (nuevo == null) return;
  const nombre = nuevo.trim();
  if (!nombre) { alert("El nombre no puede quedar vacío."); return; }
  const { error } = await sb.from("discos").update({ nombre }).eq("id", d.id);
  if (error) { alert(`No se pudo guardar el nombre: ${error.message || error.code || "error desconocido"}`); console.error(error); return; }
  await reload();
}
...
function renderCategoryChips() {
  ...
  const buildInto = (box) => {
    ...
    cats.forEach(c => {
      ...
      chip.textContent = c;
      chip.onclick = () => { currentCategory = c; renderCategoryChips(); renderActiveFilters(); renderGrid(); };
...
function renderStatusChips() {
  const buildInto = (box) => {
    ...
    [["Todos", null], ["No Jugado", "no_jugado"], ["En Curso", "en_curso"], ["Finalizado", "finalizado"]].forEach(([label, key]) => {
      ...
      chip.onclick = () => { currentStatus = label; renderStatusChips(); renderActiveFilters(); renderGrid(); };
      box.appendChild(chip);
    });
  };
...
function gameMatchesSearch(g, q) {
  if (g.nombre.toLowerCase().includes(q)) return true;
  if (g.categoria && g.categoria.toLowerCase().includes(q)) return true;
  if (g.genero_original && g.genero_original.toLowerCase().includes(q)) return true;
  const d = DISCOS.find(x => x.id == g.disco);
  if (d && d.nombre && d.nombre.toLowerCase().includes(q)) return true;
  const plataforma = d ? allPlatforms()[platformKeyOf(d.id)] : null;
  if (plataforma && plataforma.label.toLowerCase().includes(q)) return true;
  return false;
}
function getFiltered() {
  ...
  if (currentSearch.trim()) {
    ...
    list = list.filter(g => gameMatchesSearch(g, q));
  }
  if (currentPriceMin != null) list = list.filter(g => g.precio != null && Number(g.precio) >= currentPriceMin);
  if (currentPriceMax != null) list = list.filter(g => g.precio != null && Number(g.precio) <= currentPriceMax);
  switch (currentSort) {
    ...
    // Los juegos sin precio cargado quedan al final, sin importar la dirección.
    case "precio-asc": list.sort((a,b)=> (a.precio==null)-(b.precio==null) || (a.precio - b.precio) || a.nombre.localeCompare(b.nombre)); break;
    case "precio-desc": list.sort((a,b)=> (a.precio==null)-(b.precio==null) || (b.precio - a.precio) || a.nombre.localeCompare(b.nombre)); break;
  }
  return list;
}
...
// ---------- Presets de precio (estilo Mercado Libre: "Hasta $X", "$X - $Y", "Más de $X") ----------
const PRICE_PRESETS = [
  { label: "Hasta $1.000", min: null, max: 1000 },
...
function renderPricePresets() {
  const box = $("#pricePresets");
  box.innerHTML = "";
  PRICE_PRESETS.forEach(p => {
    const active = currentPriceMin === p.min && currentPriceMax === p.max;
    const chip = document.createElement("div");
    chip.className = "chip" + (active ? " active" : "");
    chip.textContent = p.label;
    chip.onclick = () => {
      // Tocar el mismo preset activo lo desactiva (vuelve a "todos los precios").
      if (active) { currentPriceMin = null; currentPriceMax = null; }
      else { currentPriceMin = p.min; currentPriceMax = p.max; }
      syncPriceInputsFromState();
      renderPricePresets(); renderActiveFilters(); renderGrid();
    };
    box.appendChild(chip);
  });
}
function syncPriceInputsFromState() {
  $("#priceMinInput").value = currentPriceMin != null ? currentPriceMin : "";
  $("#priceMaxInput").value = currentPriceMax != null ? currentPriceMax : "";
  $("#clearPriceFilterBtn").style.display = (currentPriceMin != null || currentPriceMax != null) ? "inline-flex" : "none";
}

// ---------- Barra de filtros activos (chips removibles, estilo Mercado Libre) ----------
function renderActiveFilters() {
  const bar = $("#activeFiltersBar");
  const chips = [];
  if (currentSearch.trim()) {
    chips.push({ label: `Búsqueda: "${currentSearch.trim()}"`, onRemove: () => { currentSearch = ""; $("#searchInput").value = ""; } });
  }
  if (currentCategory !== "Todas") {
    chips.push({ label: `Categoría: ${currentCategory}`, onRemove: () => { currentCategory = "Todas"; renderCategoryChips(); } });
  }
  if (currentStatus !== "Todos") {
    chips.push({ label: `Estado: ${currentStatus}`, onRemove: () => { currentStatus = "Todos"; renderStatusChips(); } });
  }
  if (currentPriceMin != null || currentPriceMax != null) {
    const label = currentPriceMin != null && currentPriceMax != null ? `Precio: ${formatPrice(currentPriceMin)} - ${formatPrice(currentPriceMax)}`
      : currentPriceMin != null ? `Precio: desde ${formatPrice(currentPriceMin)}`
      : `Precio: hasta ${formatPrice(currentPriceMax)}`;
    chips.push({ label, onRemove: () => { currentPriceMin = null; currentPriceMax = null; syncPriceInputsFromState(); renderPricePresets(); } });
  }
  ...
  chips.forEach(c => {
    ...
    const btn = document.createElement("button");
    btn.type = "button"; btn.title = "Quitar filtro"; btn.textContent = "✕";
    btn.onclick = () => { c.onRemove(); renderActiveFilters(); renderGrid(); };
    chip.appendChild(btn);
    bar.appendChild(chip);
  });
  const clearAll = document.createElement("button");
  clearAll.type = "button"; clearAll.className = "clear-all-filters-btn";
  clearAll.textContent = "Limpiar todo";
  clearAll.onclick = () => {
    currentSearch = ""; $("#searchInput").value = "";
    currentCategory = "Todas"; renderCategoryChips();
    currentStatus = "Todos"; renderStatusChips();
    currentPriceMin = null; currentPriceMax = null; syncPriceInputsFromState(); renderPricePresets();
    renderActiveFilters(); renderGrid();
  };
  bar.appendChild(clearAll);
}

// ---------- Sugerencias de búsqueda (autocompletar, estilo Mercado Libre) ----------
function renderSearchSuggestions(query) {
  const box = $("#searchSuggestions");
  const q = query.toLowerCase().trim();
  if (!q) { box.classList.remove("open"); box.innerHTML = ""; return; }
  const scope = onlyBrokenImages ? JUEGOS.filter(gameHasBrokenImage) : gamesInScope();
  const matches = scope.filter(g => gameMatchesSearch(g, q));
  box.innerHTML = "";
  if (!matches.length) {
    box.innerHTML = `<div class="search-suggestion-empty">Sin resultados para "${query}"</div>`;
  ...
  shown.forEach(g => {
    ...
    item.innerHTML = `<span class="search-suggestion-name">${g.nombre}</span><span class="search-suggestion-meta">${(g.precio != null && isEditor) ? formatPrice(g.precio) + " · " : ""}${d ? d.nombre : ""}</span>`;
    item.onclick = () => {
  ...
  if (matches.length > shown.length) {
    ...
    more.textContent = `+${matches.length - shown.length} resultado(s) más — presioná Enter para ver todos en la grilla`;
    box.appendChild(more);
  }
  box.classList.add("open");
}
function closeSearchSuggestions() { $("#searchSuggestions").classList.remove("open"); }

// Línea de precio de cada tarjeta: visible para editor principal y revendedor.
// Si ya tiene precio, muestra el monto + lápiz para cambiarlo; si no tiene,
// muestra un botón para cargarlo.
function priceLineHtml(g) {
  if (!(isEditor || isRevendedor)) return "";
  if (g.precio != null) {
    return `<div class="gcard-price">${formatPrice(g.precio)}<button class="price-edit-btn" data-price-edit="${g.id}" title="Cambiar precio" onclick="event.stopPropagation()">✏️</button></div>`;
  }
  return `<div class="gcard-price gcard-price-empty"><button data-price-edit="${g.id}" onclick="event.stopPropagation()">💲 Poner precio</button></div>`;
}
...
function renderGrid() {
  ...
  list.forEach(g => {
    ...
    card.className = "gcard" + (selectionMode ? " selectable" : "") + (selectionMode && SELECTED_IDS.has(g.id) ? " selected" : "");
    card.onclick = () => {
      if (selectionMode) { toggleSelectedGame(g.id); return; }
      openModal(g);
    };
    ...
    if (selectionMode) {
      ...
      selBox.className = "gcard-select-box";
      selBox.textContent = "✓";
      coverWrap.appendChild(selBox);
    }
    ...
    // Stock: solo el editor (clave general) puede tocarlo para cambiarlo;
    // a los demás (revendedores/visitantes) les aparece igual que siempre, sin poder tocarlo.
    const enStock = g.en_stock !== false;
    const stockBadge = document.createElement("div");
    stockBadge.className = "badge-stock " + (enStock ? "in-stock" : "out-stock");
    stockBadge.textContent = enStock ? "✅ En stock" : "❌ Sin stock";
    if (isEditor) {
      stockBadge.title = "Tocá para cambiar el stock";
      stockBadge.onclick = (e) => { e.stopPropagation(); toggleStock(g.id, !enStock); };
    }
    coverWrap.appendChild(stockBadge);
    ...
    const actionBtn = waUrl
      ? `<a class="gcard-download is-whatsapp" href="${waUrl}" target="_blank" rel="noopener" onclick="event.stopPropagation()">${WHATSAPP_ICON_SVG}Encargar</a>`
    ...
    body.innerHTML = `
      ...
      ${priceLineHtml(g)}
      <div class="gcard-actions">${selectionMode ? "" : actionBtn + cartBtn}</div>
    `;
...
// ---------- Selección múltiple para poner precio en lote (solo editor) ----------
function toggleSelectionMode() {
  selectionMode = !selectionMode;
  if (!selectionMode) SELECTED_IDS.clear();
  $("#selectModeBtn").textContent = selectionMode ? "✖️ Salir de selección" : "🧮 Seleccionar juegos";
  $("#selectModeBtn").classList.toggle("editor-active", selectionMode);
  renderBulkBar();
  renderGrid();
}
...
function renderBulkBar() {
  const bar = $("#bulkPriceBar");
  bar.style.display = selectionMode ? "flex" : "none";
  $("#bulkSelectedCount").textContent = `${SELECTED_IDS.size} juego${SELECTED_IDS.size === 1 ? "" : "s"} seleccionado${SELECTED_IDS.size === 1 ? "" : "s"}`;
}
function selectAllVisible() {
  currentList.forEach(g => SELECTED_IDS.add(g.id));
  renderBulkBar();
  renderGrid();
}
// Edición rápida de un solo precio (tarjeta o modal) — la usan tanto el
// editor principal como el revendedor. La base de datos igual valida del
// lado del servidor que un revendedor solo toque esta columna.
async function quickEditPrice(gameId) {
  if (!isEditor && !isRevendedor) return;
  const g = JUEGOS.find(x => x.id === gameId);
  if (!g) return;
  const hint = (isRevendedor && g.precio_base != null) ? ` (no puede ser menor a ${formatPrice(g.precio_base)})` : "";
  const raw = window.prompt(`Precio para "${g.nombre}"${hint} (dejalo vacío para sacarlo):`, g.precio != null ? g.precio : "");
  if (raw === null) return;
  const trimmed = raw.trim();
  const precio = trimmed === "" ? null : Number(trimmed);
  if (precio != null && (!Number.isFinite(precio) || precio < 0)) {
    alert("El precio tiene que ser un número mayor o igual a 0 (o dejalo vacío para sacarlo).");
    return;
  }
  if (isRevendedor && precio != null && g.precio_base != null && precio < g.precio_base) {
    alert(`Como revendedor no podés poner un precio menor al de la página principal (${formatPrice(g.precio_base)}).`);
    return;
  }
  const { error } = await sb.from("juegos").update({ precio }).eq("id", gameId);
  if (error) { alert(`No se pudo guardar el precio: ${error.message || error.code || "error desconocido"}`); console.error(error); return; }
  await reload();
}

async function applyBulkPrice() {
  const raw = $("#bulkPriceInput").value.trim();
  if (raw === "") { alert("Ingresá un precio para aplicar a la selección."); return; }
  const precio = Number(raw);
  if (!Number.isFinite(precio) || precio < 0) { alert("El precio tiene que ser un número mayor o igual a 0."); return; }
  if (SELECTED_IDS.size === 0) { alert("No seleccionaste ningún juego todavía. Tocá las carátulas para marcarlas."); return; }
  const ids = [...SELECTED_IDS];
  if (isRevendedor) {
    const conflictivos = JUEGOS.filter(g => ids.includes(g.id) && g.precio_base != null && precio < g.precio_base);
    if (conflictivos.length) {
      const nombres = conflictivos.slice(0, 5).map(g => g.nombre).join(", ");
      alert(`Como revendedor no podés poner ese precio: es menor al de la página principal en ${conflictivos.length} juego(s) seleccionado(s) (por ejemplo: ${nombres}). Subí el precio o deseleccioná esos juegos.`);
      return;
    }
  }
  const { error } = await sb.from("juegos").update({ precio }).in("id", ids);
  if (error) { alert(`No se pudo actualizar el precio: ${error.message || error.code || "error desconocido"}`); console.error(error); return; }
  await reload();
...
async function toggleStock(id, newValue) {
  if (!isEditor) return; // por las dudas: aunque el botón no se muestre, nunca escribe sin sesión de editor
  const { error } = await sb.from("juegos").update({ en_stock: newValue }).eq("id", id);
  if (error) { alert(`No se pudo actualizar el stock: ${error.message || error.code || "error desconocido"}`); console.error(error); return; }
  await reload();
  if (currentModalGameId === id) {
    const g = JUEGOS.find(x => x.id === id);
    if (g) openModal(g);
  }
}
...
function openModal(g, opts = {}) {
  ...
  if (isEditor || isRevendedor) {
    $("#modalPrecio").textContent = g.precio != null ? formatPrice(g.precio) : "Sin precio";
    $("#modalPrecioWrap").style.display = "";
    const precioEditBtn = $("#modalPrecioEditBtn");
    if (precioEditBtn) { precioEditBtn.style.display = "inline-flex"; precioEditBtn.onclick = () => quickEditPrice(g.id); }
  } else {
    $("#modalPrecioWrap").style.display = "none";
  }

  // Stock: visible para todos (revendedores incluidos); el botón para cambiarlo
  // es admin-only y ya lo oculta el CSS si no hay sesión de editor.
  const enStockModal = g.en_stock !== false;
  $("#modalStock").textContent = enStockModal ? "✅ En stock" : "❌ Sin stock";
  $("#modalStockWrap").style.display = "";
  const stockToggleBtn = $("#modalStockToggleBtn");
  stockToggleBtn.textContent = enStockModal ? "📦 Marcar sin stock" : "📦 Marcar en stock";
  stockToggleBtn.classList.toggle("out-stock", !enStockModal);
  stockToggleBtn.style.display = "";
  stockToggleBtn.onclick = () => toggleStock(g.id, !enStockModal);

  const dlBtn = $("#modalDownloadBtn");
  const waUrl = whatsappOrderUrl(g.disco, g.nombre);
  dlBtn.classList.remove("is-whatsapp");
  if (waUrl) { dlBtn.href = waUrl; dlBtn.innerHTML = WHATSAPP_ICON_SVG + "Encargar por WhatsApp"; dlBtn.classList.add("is-whatsapp"); dlBtn.style.display = "inline-block"; }
...
function fillForm(g) {
  ...
  $("#inputPrecio").value = (g.precio != null) ? g.precio : "";
  const inputPrecioBase = $("#inputPrecioBase");
  if (inputPrecioBase) inputPrecioBase.value = (g.precio_base != null) ? g.precio_base : "";
...
function openNewGameModal() {
  ...
  $("#modalPrecioWrap").style.display = "none";
  $("#modalStockWrap").style.display = "none";
  $("#modalStockToggleBtn").style.display = "none";
  $("#modalDownloadBtn").classList.remove("is-whatsapp");
...
function selectConsola(key) {
  ...
  renderConsolaSelect(); renderDiscTabs(); renderBanner(); renderCategoryChips(); renderStatusChips(); renderActiveFilters(); renderGrid();
...
async function saveGame() {
  ...
  const payload = {
    ...
    precio: $("#inputPrecio").value.trim() === "" ? null : Number($("#inputPrecio").value),
    precio_base: $("#inputPrecioBase").value.trim() === "" ? null : Number($("#inputPrecioBase").value),
  ...
  if (payload.precio != null && (!Number.isFinite(payload.precio) || payload.precio < 0)) {
    alert("El precio tiene que ser un número mayor o igual a 0 (o dejalo vacío si no tiene).");
    return;
  }
  if (payload.precio_base != null && (!Number.isFinite(payload.precio_base) || payload.precio_base < 0)) {
    alert("El precio de página principal tiene que ser un número mayor o igual a 0 (o dejalo vacío si no tiene).");
    return;
  }
...
async function init() {
  ...
  renderConsolaSelect(); renderDiscTabs(); renderBanner(); renderGeneralDownload(); renderCategoryChips(); renderStatusChips(); renderPricePresets(); renderActiveFilters(); renderGrid();
  renderCartBadge();

  $("#searchInput").addEventListener("input", (e) => {
    currentSearch = e.target.value;
    renderSearchSuggestions(currentSearch);
    renderActiveFilters();
    renderGrid();
  });
  $("#searchInput").addEventListener("keydown", (e) => {
    if (e.key === "Enter") closeSearchSuggestions();
  });
  $("#searchInput").addEventListener("focus", (e) => { if (e.target.value.trim()) renderSearchSuggestions(e.target.value); });
  // Cerrar las sugerencias al tocar afuera del buscador (con un pequeño delay
  // para que el click sobre una sugerencia se procese antes de ocultarlas).
  document.addEventListener("click", (e) => { if (!$("#searchWrap").contains(e.target)) closeSearchSuggestions(); });
  $("#sortSelect").addEventListener("change", (e) => { currentSort = e.target.value; renderGrid(); });
  $("#priceMinInput").addEventListener("input", (e) => {
    currentPriceMin = e.target.value.trim() === "" ? null : Number(e.target.value);
    $("#clearPriceFilterBtn").style.display = (currentPriceMin != null || currentPriceMax != null) ? "inline-flex" : "none";
    renderPricePresets(); renderActiveFilters(); renderGrid();
  });
  $("#priceMaxInput").addEventListener("input", (e) => {
    currentPriceMax = e.target.value.trim() === "" ? null : Number(e.target.value);
    $("#clearPriceFilterBtn").style.display = (currentPriceMin != null || currentPriceMax != null) ? "inline-flex" : "none";
    renderPricePresets(); renderActiveFilters(); renderGrid();
  });
  $("#clearPriceFilterBtn").addEventListener("click", () => {
    currentPriceMin = null; currentPriceMax = null;
    syncPriceInputsFromState(); renderPricePresets(); renderActiveFilters(); renderGrid();
  });
  $("#selectModeBtn").addEventListener("click", toggleSelectionMode);
  $("#bulkApplyPriceBtn").addEventListener("click", applyBulkPrice);
  $("#bulkSelectAllBtn").addEventListener("click", selectAllVisible);
  $("#bulkCancelBtn").addEventListener("click", toggleSelectionMode);
  ...
  $("#unlockRevendedorBtn").addEventListener("click", unlockRevendedor);
  $("#revendedoresPanelBtn").addEventListener("click", openRevendedoresPanel);
  $("#closeRevendedoresModal").addEventListener("click", closeRevendedoresPanel);
  $("#revendedoresModalOverlay").addEventListener("click", (e) => { if (e.target.id === "revendedoresModalOverlay") closeRevendedoresPanel(); });
  $("#revendedoresList").addEventListener("click", (e) => {
    const btn = e.target.closest("[data-save-disco]");
    if (btn) saveRevendedorRow(btn.dataset.saveDisco);
  });
  const revAccountsList = $("#revendedoresAccountsList");
  if (revAccountsList) {
    revAccountsList.addEventListener("click", (e) => {
      const btn = e.target.closest("[data-toggle-revendedor]");
      if (btn) toggleRevendedorActivo(btn.dataset.toggleRevendedor, btn.dataset.activo === "true");
    });
  }
  const crearRevendedorBtn = $("#crearRevendedorBtn");
  if (crearRevendedorBtn) crearRevendedorBtn.addEventListener("click", submitNuevoRevendedor);
  ...
  grid.addEventListener("click", (e) => {
    const priceBtn = e.target.closest("[data-price-edit]");
    if (priceBtn) { e.stopPropagation(); quickEditPrice(priceBtn.dataset.priceEdit); return; }

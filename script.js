...
function applyEditorUI() {
  ...
  $("#sortPrecioAsc").style.display = isEditor ? "" : "none";
  $("#sortPrecioDesc").style.display = isEditor ? "" : "none";
  renderActiveFilters();
  renderGrid();
}
...
function renderGrid() {
  ...
  list.forEach(g => {
    ...
    body.innerHTML = `
      ...
      ${g.precio != null && isEditor ? `<div class="gcard-price">${formatPrice(g.precio)}</div>` : ""}
...
function openModal(g, opts = {}) {
  ...
  if (g.precio != null && isEditor) { $("#modalPrecio").textContent = formatPrice(g.precio); $("#modalPrecioWrap").style.display = ""; }

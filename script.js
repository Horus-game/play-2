...
// Icono de WhatsApp en SVG (para no depender de una fuente de íconos externa).
const WHATSAPP_ICON_SVG = `<svg class="wa-icon" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M16.004 2.667c-7.363 0-13.333 5.97-13.333 13.333 0 2.352.615 4.646 1.784 6.667L2.667 29.333l6.822-1.788a13.28 13.28 0 0 0 6.515 1.72h.006c7.362 0 13.333-5.97 13.333-13.333S23.366 2.667 16.004 2.667zm0 24.4h-.005a11.05 11.05 0 0 1-5.636-1.543l-.404-.24-4.05 1.062 1.082-3.948-.263-.406a11.02 11.02 0 0 1-1.694-5.892c0-6.105 4.97-11.074 11.076-11.074 2.958 0 5.74 1.153 7.833 3.248a11 11 0 0 1 3.238 7.83c-.002 6.106-4.972 11.075-11.077 11.075zm6.073-8.29c-.333-.167-1.966-.97-2.27-1.08-.305-.111-.527-.167-.75.167-.222.333-.86 1.08-1.055 1.302-.194.222-.388.25-.72.083-.334-.167-1.41-.52-2.687-1.657-.994-.886-1.664-1.98-1.86-2.313-.194-.333-.02-.514.146-.68.15-.15.334-.389.5-.583.167-.194.222-.333.334-.556.11-.222.055-.416-.028-.583-.083-.167-.75-1.807-1.028-2.474-.27-.65-.545-.562-.75-.572-.194-.01-.417-.012-.64-.012-.222 0-.583.083-.888.417-.305.333-1.166 1.14-1.166 2.78s1.194 3.226 1.36 3.448c.166.222 2.352 3.593 5.7 5.04.797.344 1.418.55 1.902.703.799.254 1.526.218 2.101.132.641-.096 1.966-.804 2.244-1.58.277-.777.277-1.443.194-1.58-.083-.14-.305-.222-.638-.39z"/></svg>`;
...
async function init() {
  ...
  const updateClearPriceBtn = () => { $("#clearPriceFilterBtn").style.display = (currentPriceMin != null || currentPriceMax != null) ? "inline-flex" : "none"; };
  $("#priceMinInput").addEventListener("input", (e) => {
    currentPriceMin = e.target.value.trim() === "" ? null : Number(e.target.value);
    updateClearPriceBtn(); renderGrid();
  });

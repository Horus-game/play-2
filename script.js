...
let isEditor = false;
// ---- Modo revendedor (?rev=slug): precios y WhatsApp propios de cada revendedor ----
const REV_SLUG = new URLSearchParams(location.search).get("rev");
let REV = null;        // datos públicos del revendedor (nombre, whatsapp)
let REV_CLAVE = null;  // clave del revendedor, solo en memoria mientras edita sus precios
const canSeePrices = () => isEditor || !!REV_SLUG;
...
async function loadData() {
  ...
  JUEGOS = juegos || [];
  if (REV_SLUG) {
    const [{ data: rv }, { data: pr }] = await Promise.all([
      sb.from("revendedores_publico").select("*").eq("slug", REV_SLUG).maybeSingle(),
      sb.from("precios_revendedor").select("juego_id,precio").eq("slug", REV_SLUG),
    ]);
    REV = rv || null;
    const mapa = new Map((pr || []).map(x => [x.juego_id, Number(x.precio)]));
    JUEGOS = JUEGOS.map(g => ({ ...g, precio: mapa.has(g.id) ? mapa.get(g.id) : null }));
  }
...
async function unlockReseller() {
  if (REV_CLAVE) { REV_CLAVE = null; applyEditorUI(); await reload(); return; }
  const pass = window.prompt("Clave del revendedor:");
  if (!pass) return;
  const { data, error } = await sb.rpc("rev_login", { p_slug: REV_SLUG, p_clave: pass });
  if (error || !data) { alert("Clave incorrecta."); return; }
  REV_CLAVE = pass;
  applyEditorUI();
  await reload();
}

// Admin: crea (o actualiza) un revendedor y muestra su link
async function newReseller() {
  if (!isEditor) return;
  const slug = (prompt("Identificador para el link del revendedor (ej: juanperez):") || "").trim().toLowerCase().replace(/[^a-z0-9-]/g, "");
  if (!slug) return;
  const nombre = (prompt("Nombre a mostrar en su página:") || "").trim();
  if (!nombre) return;
  const whatsapp = (prompt("WhatsApp con código de país, solo números (ej: 5493584123456):") || "").replace(/[^0-9]/g, "");
  if (!whatsapp) return;
  const clave = prompt("Clave que usará el revendedor para cargar sus precios:");
  if (!clave) return;
  const { error } = await sb.rpc("admin_crear_revendedor", { p_slug: slug, p_nombre: nombre, p_whatsapp: whatsapp, p_clave: clave });
  if (error) { alert("No se pudo crear el revendedor: " + (error.message || "error desconocido")); return; }
  prompt("Revendedor guardado. Este es su link (copialo y pasáselo):", `${location.origin}${location.pathname}?rev=${slug}&consola=${currentConsola}&disco=${ALL_ID}`);
}

async function unlockEditor() {
  if (REV_SLUG) return unlockReseller();
  if (isEditor) {
...
function applyEditorUI() {
  ...
  document.body.classList.toggle("rev-mode", !!REV_SLUG);
  document.body.classList.toggle("rev-editing", !!REV_CLAVE);
  if (REV_SLUG) {
    $("#unlockBtn").textContent = REV_CLAVE ? "🔓 Editando mis precios (salir)" : "🔒 Mis precios";
    $("#unlockBtn").classList.toggle("editor-active", !!REV_CLAVE);
  }
  updateDeletePlatformBtn();
  // Al salir del modo edición: apagar el modo "seleccionar juegos" si estaba
  // activo, y limpiar cualquier filtro/orden por precio (los precios quedan
  // ocultos para quien no está editando, así que no tiene sentido dejarlos aplicados).
  if (!isEditor && !REV_CLAVE && selectionMode) { selectionMode = false; SELECTED_IDS.clear(); renderBulkBar(); }
  if (!canSeePrices()) {
  ...
  $("#sortPrecioAsc").style.display = canSeePrices() ? "" : "none";
  $("#sortPrecioDesc").style.display = canSeePrices() ? "" : "none";
  renderActiveFilters();
  renderGrid();
}
...
function whatsappNumeroOf(discId) {
  if (REV_SLUG) return REV && REV.whatsapp ? REV.whatsapp : null;
  const d = DISCOS.find(x => x.id == discId);
...
function whatsappOrderUrl(discId, nombreJuego) {
  ...
  const texto = encodeURIComponent(`Quisiera encargar este juego: ${nombreJuego}${(() => { const pg = REV_SLUG && JUEGOS.find(x => x.disco == discId && x.nombre === nombreJuego); return pg && pg.precio != null ? " (" + formatPrice(pg.precio) + ")" : ""; })()}`);
  return `https://wa.me/${numero}?text=${texto}`;
}
// --- Carrito de pedido (persistido en este navegador con localStorage) ---
const CART_STORAGE_KEY = "ps2gamepass_cart" + (REV_SLUG ? "_" + REV_SLUG : "");
let CART = new Set();
function loadCart() {
  try {
...
function renderCartModal() {
  ...
  groups.forEach((games, numero) => {
    const total = REV_SLUG ? games.reduce((t, g) => t + (Number(g.precio) || 0), 0) : 0;
    const texto = encodeURIComponent(`Quisiera encargar estos juegos:\n${games.map(g => "- " + g.nombre + (REV_SLUG && g.precio != null ? " — " + formatPrice(g.precio) : "")).join("\n")}` + (total ? `\nTotal: ${formatPrice(total)}` : ""));
...
function getFiltered() {
  ...
  if (REV_SLUG && !REV_CLAVE) list = list.filter(g => g.precio != null); // el cliente solo ve lo que el revendedor puso a la venta
...
function renderSearchSuggestions(query) {
  ...
  shown.forEach(g => {
    ...
    item.innerHTML = `<span class="search-suggestion-name">${g.nombre}</span><span class="search-suggestion-meta">${(g.precio != null && canSeePrices()) ? formatPrice(g.precio) + " · " : ""}${d ? d.nombre : ""}</span>`;
    item.onclick = () => {
...
function renderGrid() {
  ...
  list.forEach(g => {
    ...
    body.innerHTML = `
      ...
      ${g.precio != null && canSeePrices() ? `<div class="gcard-price">${formatPrice(g.precio)}</div>` : ""}
...
async function applyBulkPrice() {
  ...
  if (raw === "" && !REV_CLAVE) { alert("Ingresá un precio para aplicar a la selección."); return; }
  const precio = raw === "" ? null : Number(raw); // revendedor: vacío = quitar precio
  if (precio != null && (!Number.isFinite(precio) || precio < 0)) { alert("El precio tiene que ser un número mayor o igual a 0."); return; }
  ...
  const { error } = REV_CLAVE
    ? await sb.rpc("rev_set_precios", { p_slug: REV_SLUG, p_clave: REV_CLAVE, p_items: ids.map(id => ({ juego_id: id, precio })) })
    : await sb.from("juegos").update({ precio }).in("id", ids);
...
function openModal(g, opts = {}) {
  ...
  if (g.precio != null && canSeePrices()) { $("#modalPrecio").textContent = formatPrice(g.precio); $("#modalPrecioWrap").style.display = ""; }
  else { $("#modalPrecioWrap").style.display = "none"; }
...
async function init() {
  ...
  await loadData();
  if (REV_SLUG) {
    if (!REV) { document.body.innerHTML = '<p style="padding:2rem;font-family:sans-serif">Este revendedor no existe o el enlace es incorrecto.</p>'; return; }
    document.title = REV.nombre;
    const h = document.querySelector("h1"); if (h) h.textContent = REV.nombre;
  }
  ...
  $("#newRevBtn").addEventListener("click", newReseller);

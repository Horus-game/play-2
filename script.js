...
async function reload() {
  ...
  renderStatusChips();
  renderGrid();
  if (selectMode) { renderMoveTargetSelect(); updateSelectBar(); }
...
function applyEditorUI() {
  ...
  if (!isEditor && selectMode) exitSelectMode();
}

// ---------- Selección múltiple (mover / eliminar varios juegos) ----------
...
function enterSelectMode() {
  ...
  $("#selectModeBtn").textContent = "✕ Salir de selección";
  renderMoveTargetSelect();
  updateSelectBar();
  renderGrid();
}
...
function exitSelectMode() {
  ...
  $("#selectModeBtn").textContent = "☑️ Seleccionar";
  renderGrid();
}
...
function selectAllVisible() {
  const allSelected = currentList.length > 0 && currentList.every(g => selectedIds.has(g.id));
  if (allSelected) {
    currentList.forEach(g => selectedIds.delete(g.id));
  } else {
    currentList.forEach(g => selectedIds.add(g.id));
  }
  updateSelectBar();
  renderGrid();
}
...
async function moveSelectedGames() {
  if (selectedIds.size === 0) return;
  const targetId = $("#moveTargetSelect").value;
  if (!targetId) return;
  const targetName = DISCOS.find(d => String(d.id) === String(targetId))?.nombre || targetId;
  const ids = Array.from(selectedIds);
  if (!confirm(`¿Mover ${ids.length} juego(s) a "${targetName}"?`)) return;
  const { error } = await sb.from("juegos").update({ disco: targetId }).in("id", ids);
  if (error) { alert("No se pudieron mover los juegos. ¿Iniciaste sesión con la clave?"); console.error(error); return; }
  selectedIds.clear();
  await reload();
  updateSelectBar();
}
...
async function deleteSelectedGames() {
  if (selectedIds.size === 0) return;
  const ids = Array.from(selectedIds);
  if (!confirm(`¿Eliminar ${ids.length} juego(s) del catálogo? Esta acción no se puede deshacer.`)) return;
  const { error } = await sb.from("juegos").delete().in("id", ids);
  if (error) { alert("No se pudieron eliminar los juegos. ¿Iniciaste sesión con la clave?"); console.error(error); return; }
  selectedIds.clear();
  await reload();
  updateSelectBar();
}
...
function renderGrid() {
  ...
  list.forEach(g => {
    ...
    card.className = "gcard" + (selectedIds.has(g.id) ? " selected" : "");
    card.onclick = () => {
      if (selectMode) { toggleSelectGame(g.id); return; }
      openModal(g);
    };
    ...
    selectBox.className = "gcard-select-box";
    selectBox.textContent = "✓";
    coverWrap.appendChild(selectBox);
...
function closeModalUI() { $("#modalOverlay").classList.remove("open"); currentModalGameId = null; isCreatingNew = false; }
...
// si hay una entrada de historial nuestra, retrocedemos (eso dispara popstate,
// que a su vez llama a la función *UI* real); si no, cerramos directo.
...
- caratula: OPCIONAL. Si sabés un link DIRECTO a una imagen de carátula (termina en .jpg/.png/.webp, no una página de Google/Bing), ponelo. Si no estás seguro, dejalo vacío ("") — el sitio puede buscarla después automáticamente.
...
[{"nombre":"God of War","categoria":"Aventura","genero_original":"Hack and slash","dificultad":4,"jugadores":"1","resena":"Kratos busca venganza contra los dioses del Olimpo en esta épica aventura de acción mitológica.","estado":"no_jugado","caratula":""}]
...
async function importGamesFromJson() {
  ...
  const rows0 = parsed.map(item => {
    ...
    return {
      ...
      caratula: String(item.caratula || "").trim() || null,
    };
  }).filter(Boolean);

  if (!rows0.length) { $("#importStatusMsg").textContent = "❌ No se encontró ningún juego válido en ese JSON."; return; }

  // --- Validación de URLs de carátula, igual que en el formulario de un solo
  // juego: si el link es claramente una página de búsqueda (rompería la card
  // en el catálogo), no se guarda NADA de la lista hasta que se corrija. Si es
  // solo "dudoso" (formato desconocido), se avisa y se pide confirmar antes de
  // guardar el lote completo. ---
  const brokenCovers = rows0.filter(r => isLikelyDirectImageUrl(r.caratula) === false);
  if (brokenCovers.length) {
    $("#importStatusMsg").textContent = `❌ No se guardó nada. Estos juegos tienen un link de carátula que es una página de búsqueda (Google/Bing/DuckDuckGo), no una imagen directa, y no se van a ver en el catálogo: ${brokenCovers.map(r => r.nombre).join(", ")}. Corregí esos links (o dejalos vacíos) en el JSON pegado y volvé a intentar.`;
    return;
  }
  const uncertainCovers = rows0.filter(r => isLikelyDirectImageUrl(r.caratula) === null);
  if (uncertainCovers.length && !confirm(`${uncertainCovers.length} juego(s) tienen un link de carátula con un formato que no reconozco (puede que no se vea en el catálogo): ${uncertainCovers.map(r => r.nombre).join(", ")}.

¿Guardar igual toda la lista?`)) {
    return;
  }
...
async function init() {
  ...
  $("#btnMoveSelected").addEventListener("click", moveSelectedGames);
  $("#btnDeleteSelected").addEventListener("click", deleteSelectedGames);

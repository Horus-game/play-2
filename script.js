...
function fillForm(g) {
  ...
  const opciones = DISCOS.map(d => `<option value="${d.id}" ${d.id==g.disco?"selected":""}>${d.nombre}</option>`);
  if (g.disco && !discoExiste) {
    opciones.unshift(`<option value="${g.disco}" selected>⚠️ Disco "${g.disco}" ya no existe — elegí uno</option>`);
  }
...
function openDiscModal(d, prefillPlatform) {
  ...
  const plats = allPlatforms();
  $("#discInputPlataforma").innerHTML = Object.entries(plats)
    .map(([key, p]) => `<option value="${key}">${p.icon} ${p.label}</option>`).join("")
    + `<option value="__custom__">➕ Otra consola (agregar nueva)...</option>`;
  $("#discInputPlataforma").value = (d && d.plataforma && plats[d.plataforma]) ? d.plataforma
...
async function importGamesFromJson() {
  ...
  // Evitar ids duplicados: relee los ids reales de la base (no solo lo que
  // ya está cargado en memoria) para no chocar con registros insertados
  // desde otra pestaña/sesión o en un intento anterior.
  $("#importStatusMsg").textContent = "Verificando duplicados...";
  const { data: existingRows, error: existingErr } = await sb.from("juegos").select("id");
  if (existingErr) {
    $("#importStatusMsg").textContent = `❌ No se pudo verificar duplicados: ${existingErr.message || existingErr.code}`;

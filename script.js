...
function applySession(session) {
  ...
  isRevendedor = !!session && email === window.REVENDEDOR_EMAIL;
}
...
// intente saltear la web (ver migracion_revendedor.sql).
async function unlockRevendedor() {
  if (isRevendedor) {
    await sb.auth.signOut();
    return;
  }
  const pass = window.prompt("Clave de revendedor:");
  if (!pass) return;
  const { error } = await sb.auth.signInWithPassword({ email: window.REVENDEDOR_EMAIL, password: pass });
  if (error) {
    alert("Clave incorrecta o falta configurar el usuario \"revendedor\" en Supabase (ver README).");
    console.error(error);
  }
}

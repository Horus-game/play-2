...
// Trae TODAS las filas de una tabla, sin el límite implícito de 1000 filas
// que impone Supabase/PostgREST por defecto en cada select(). Pagina de a
// 1000 hasta que ya no vengan más filas.
async function fetchAllRows(table, columns = "*") {
  ...
  while (true) {
    ...
    if (error) { console.error(`Error trayendo ${table}:`, error); break; }
    all = all.concat(data || []);
...
async function loadData() {
  if (!sb) return;
  const [juegos, discos, plataformas, { data: config }] = await Promise.all([
    fetchAllRows("juegos"),
    fetchAllRows("discos"),
    fetchAllRows("plataformas"),
...
function openCartModal() { renderCartModal(); $("#cartModalOverlay").classList.add("open"); }
...
// --- Bloqueo de scroll de fondo (mobile) ---
// position:fixed en <body> hace perder el scroll actual, así que lo guardamos
// y lo restauramos al desbloquear, para que la página no salte al inicio.
let _savedScrollY = 0;

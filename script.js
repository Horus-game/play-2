...
const PLATFORMS = {
  ps2:       { label: "PlayStation 2",          libretro: "Sony_-_PlayStation_2" },
  ps1:       { label: "PlayStation 1",          libretro: "Sony_-_PlayStation" },
  ps3:       { label: "PlayStation 3",          libretro: null },
  psp:       { label: "PSP",                    libretro: "Sony_-_PlayStation_Portable" },
  xbox:      { label: "Xbox",                   libretro: "Microsoft_-_Xbox" },
  xbox360:   { label: "Xbox 360",                libretro: "Microsoft_-_Xbox_360" },
  gamecube:  { label: "GameCube",               libretro: "Nintendo_-_GameCube" },
  wii:       { label: "Wii",                     libretro: "Nintendo_-_Wii" },
  n64:       { label: "Nintendo 64",             libretro: "Nintendo_-_Nintendo_64" },
  snes:      { label: "Super Nintendo",          libretro: "Nintendo_-_Super_Nintendo_Entertainment_System" },
  nes:       { label: "Nintendo (NES)",          libretro: "Nintendo_-_Nintendo_Entertainment_System" },
  gba:       { label: "Game Boy Advance",        libretro: "Nintendo_-_Game_Boy_Advance" },
  switch:    { label: "Nintendo Switch",         libretro: null },
  dreamcast: { label: "Dreamcast",               libretro: "Sega_-_Dreamcast" },
  megadrive: { label: "Mega Drive / Genesis",    libretro: "Sega_-_Mega_Drive_-_Genesis" },
  pc:        { label: "PC",                      libretro: null },
  otro:      { label: "Otra",                    libretro: null },
};
...
// ---------- Render ----------
function renderConsolaTabs() {
  const tabs = $("#consolaTabButtons");
  const title = tabs && tabs.previousElementSibling;
  if (!tabs) return;
  tabs.innerHTML = "";
  const consolas = consolasPresentes();
  const show = consolas.length >= 2; // no tiene sentido el selector si solo hay una consola cargada
  tabs.style.display = show ? "" : "none";
  if (title) title.style.display = show ? "" : "none";
  if (!show) return;
  const allBtn = document.createElement("button");
  allBtn.className = "disc-tab" + (currentConsola === ALL_ID ? " active" : "");
  if (currentConsola === ALL_ID) allBtn.style.background = "linear-gradient(135deg,var(--accent),var(--accent2))";
  allBtn.textContent = "Todas";
  allBtn.onclick = () => selectConsola(ALL_ID);
  tabs.appendChild(allBtn);
  consolas.forEach(key => {
    const btn = document.createElement("button");
    btn.className = "disc-tab" + (key === currentConsola ? " active" : "");
    if (key === currentConsola) btn.style.background = "linear-gradient(135deg,var(--accent),var(--accent2))";
    btn.textContent = PLATFORMS[key].label;
    btn.onclick = () => selectConsola(key);
...
function selectConsola(key) {
  ...
  renderConsolaTabs(); renderDiscTabs(); renderBanner(); renderCategoryChips(); renderStatusChips(); renderGrid();
...
async function init() {
  ...
  renderConsolaTabs(); renderDiscTabs(); renderBanner(); renderGeneralDownload(); renderCategoryChips(); renderStatusChips(); renderGrid();
